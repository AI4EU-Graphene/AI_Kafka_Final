...
async def get_live_data(client, region="ALL", semaphore: asyncio.Semaphore = None):
    categories = ["demandactual", "generationactual", "windactual", "interconnection", "co2intensity", "co2emission", "SnspALL"]
    current_date = datetime.now().strftime("%d-%b-%Y+%H%%3A%M")  # format: 01-Apr-2025+14%3A00
        ...
        async with semaphore:
            try:
                ...
                if data:
                    ...
                    logging.info(f"Fetched {len(df)} live records for {category}")
                    yield df
            except Exception as e:
                logging.warning(f"Failed to fetch live data for {api}: {e}")
